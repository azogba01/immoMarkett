...
<div class="row">
  <div class="col-lg-6 grid-margin stretch-card">
    <div class="card">
      ...
      <button type='submit' class="btn btn-success" ><a href="{{url('/typebailleurcreate')}}">Ajouter</a></button>
   
            </tr>
          </thead>
          <tbody>
            @foreach ($typebailleur as $item)
    <tr>
      ...
      <td>{{$item->nom}}</td>
        ...
        <td>
        <form action="/typebailleurs/{{$item->id}}" method="POST">
           @csrf
           @method('delete')
      ...
      <td><a href="/typebailleurs/{{$item->id}}"><button class="btn btn-success">Detail</button></a></td>
 </tr>
   @endforeach
          </tbody>
        </table>
